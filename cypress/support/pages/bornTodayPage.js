export class BornTodayPage {
    ...
    clickOnSearchResultName(item) {
        cy.get('[class="ipc-metadata-list-summary-item"]').eq(item).find('[data-testid="nlib-title"] a').click();
    }
    ...
    clickOnFirstDescriptionLink(item) {
        cy.get('[class="ipc-metadata-list-summary-item"]').eq(item).find('[data-testid="dli-bio"] a').first().click();
    }
}
