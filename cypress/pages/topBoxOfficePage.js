export class TopBoxOfficePage {
    ...
    clickOnItemRateButton(item) {
        cy.get('[data-testid="chart-layout-main-column"] [class="ipc-metadata-list-summary-item"]').eq(item).then(movie => {
            cy.wrap(movie).find('[class="ipc-title__text"').then(title => { 
                ...
                cy.assertElementContainsText('[class="ipc-rating-prompt__content-title"]', movieTitle);
            })
        })
    }
    ...
    setRating(rating) {
        ...
        cy.get('[class="ipc-starbar__rating"] button').eq(arrayRatingPosition).click({force: true});
        cy.assertElementContainsText('[class="ipc-rating-display__rating"]', rating.toString());
    }

    // Clicks on rate button on rating prompt
    rate() {
        cy.get('[class="ipc-rating-prompt__rating-container"] button').contains('Rate').click();
    }
}
